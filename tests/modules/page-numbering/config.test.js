...
import {
	...
	VALID_STYLES,
	DEFAULT_CONFIG
} from "../../../src/modules/page-numbering/config.js";
...
describe("Page Numbering Configuration", () => {
	...
	describe("validateStart", () => {
		...
		test("returns false for non-positive or non-integer values", () => {
			...
			expect(validateStart(true)).toBe(false);
		});
	});
	...
	describe("validateTemplate", () => {
		...
		test("returns false for non-string values", () => {
			...
			expect(validateTemplate([])).toBe(false);
			expect(validateTemplate(true)).toBe(false);
		});
	});
	...
	describe("validateCss", () => {
		...
		test("returns false for invalid CSS objects", () => {
			expect(validateCss("font-size: 12px")).toBe(false);
			expect(validateCss(123)).toBe(false);
			expect(validateCss(true)).toBe(false);
			expect(validateCss([])).toBe(false);
			expect(validateCss({ fontSize: 12 })).toBe(false); // number instead of string
			expect(validateCss({ color: null })).toBe(false); // null instead of string
		});
	});
	...
	describe("validateClassName", () => {
		...
		test("returns false for invalid class names", () => {
			...
			expect(validateClassName(123)).toBe(false);
			expect(validateClassName({})).toBe(false);
		});
	});
	...
	describe("validateEnabled", () => {
		...
		test("returns false for non-boolean values", () => {
			expect(validateEnabled("true")).toBe(false);
			expect(validateEnabled("false")).toBe(false);
			expect(validateEnabled(1)).toBe(false);
			expect(validateEnabled(0)).toBe(false);
			expect(validateEnabled(null)).toBe(false);
			expect(validateEnabled(undefined)).toBe(false);
			expect(validateEnabled({})).toBe(false);
		});
	});
	...
	describe("validateConfig", () => {
		...
		test("returns invalid for config with invalid template", () => {
			const result = validateConfig({ template: 123 });
			expect(result.isValid).toBe(false);
		...
		test("returns invalid for config with invalid css", () => {
			const result = validateConfig({ css: "invalid" });
			expect(result.isValid).toBe(false);
	...
	describe("normalizeConfig", () => {
		...
		test("returns default config for non-object input", () => {
			...
			expect(normalizeConfig(123)).toEqual(DEFAULT_CONFIG);
		});

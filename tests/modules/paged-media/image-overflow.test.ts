 ...
 */
import ImageOverflowHandler from "../../../src/modules/paged-media/image-overflow.ts";

describe("ImageOverflowHandler", () => {
	let handler: ImageOverflowHandler;
	...
	describe("constructor", () => {
		...
		it("should have an image cache", () => {
			expect((handler as any)._imageCache).toBeDefined();
			expect((handler as any)._imageCache).toBeInstanceOf(Map);
		});

		it("should have a processed images set", () => {
			expect((handler as any)._processedImages).toBeDefined();
			expect((handler as any)._processedImages).toBeInstanceOf(WeakSet);
		});
	});
	...
	describe("_constrainImage", () => {
		it("should scale down image that exceeds width", () => {
			...
			(handler as any)._constrainImage(img, 1200, 800, 800, 600);
		...
		it("should scale down image that exceeds height", () => {
			...
			(handler as any)._constrainImage(img, 600, 1500, 800, 1000);
		...
		it("should not scale image that fits", () => {
			...
			(handler as any)._constrainImage(img, 400, 300, 800, 600);
	...
	describe("_processImage", () => {
		it("should skip images with user-specified max-width", () => {
			...
			(handler as any)._processImage(img, 800, 600);
